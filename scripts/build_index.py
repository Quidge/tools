...
def redirect_stubs() -> set[str]:
    """Filenames generated by build_redirects.py — these are redirects, not tools,
    so they must be kept out of the index even though they match *.html."""
    path = Path("_redirects.json")
    if not path.exists():
        return set()
    try:
        redirects = json.loads(path.read_text())
    except (ValueError, OSError):
        return set()
    return {f"{source}.html" for source in redirects}
...
def gather_tools() -> list[Tool]:
    ...
    tools = []
    excluded = EXCLUDED | redirect_stubs()
